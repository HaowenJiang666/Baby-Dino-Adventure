...
HEALTH = [pygame.image.load(os.path.join("Assets/Health", "Health.png")),pygame.image.load(os.path.join("Assets/Health", "Health.png")),
pygame.image.load(os.path.join("Assets/Health", "Health.png")),pygame.image.load(os.path.join("Assets/Health", "Health.png"))]
...
class Dinosaur:
    ...
    Y_POS = 90.21 + 30 # dino Y position
    Y_POS_DUCK = 98.94 + 30 # duck Y position
    ...
    def jump(self, userInput):
        ...
        if self.dino_rect.y > 120: # dino touch the ground after jump
            self.dino_jump = False
            self.dino_jump2 = False
            self.jump_vel = self.JUMP_VEL
            self.dino_rect.y = 90.21 + 30
...
class Bomb(Obstacle):
    def __init__(self, image):
        self.type = 0
        super().__init__(image, self.type)
        self.rect.y = 50.925 + 30

class River(Obstacle):
    def __init__(self, image):
        self.type = 0
        super().__init__(image, self.type)
        self.rect.y = 101.85 + 30
...
def main():
    global game_speed, x_pos_bg, y_pos_bg, points, obstacles, health, health_num, cheat, treasures, obstacle
    run = True
    cheat = False
    ...
    while run:
        ...
        if cheat:
            ...
            cheatRect.center = (200, 12.64)
            SCREEN.blit(cheat_text,cheatRect)
        userInput = pygame.key.get_pressed()
        # SCREEN.blit(HEALTH, (SCREEN_WIDTH // 2 - 80, SCREEN_HEIGHT // 2 - 116))
        ...
        if(time.time() - start_time > 3):
            ...
            for treasure in treasures:
                ...
                if treasure.rect.colliderect(obstacle.rect):
                    treasures.pop()
                if player.dino_rect.colliderect(treasure.rect):
                    if not cheat:
                        points += 100
                    treasures.pop()
